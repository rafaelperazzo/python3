...
RUN wget -O /opt/Python-3.13.4.tar.xz https://www.python.org/ftp/python/3.13.4/Python-3.13.4.tar.xz
RUN tar -xf /opt/Python-3.13.4.tar.xz -C /opt
RUN cd /opt/Python-3.13.4 && ./configure --enable-optimizations --with-ssl-default-suites=openssl && make && make install
